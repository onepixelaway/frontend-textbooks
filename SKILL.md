...
- Use browser-side measured pagination for normal body prose so the browser does the page fitting but the result is still real `.page.text-page` sheets. The bundled scaffold already does this with `paginateChapter(...)`.
...
- For editorial, HBR-like, business, field-guide, or coffee-table output, do not leave long preserved prose in `.chapter-flow`. Use measured text pages (`.page.text-page.text-two`, `.text-single`, `.text-short-single`, `.text-stack`, or `.text-tail`) for body sections. Reserve `.chapter-flow` for a deliberately plain reader edition, appendices/notes/bibliography, or a short non-editorial section, and mark the book or section with `data-allow-flowing-prose="true"` when that is intentional.
...
For any designed prose book, either start from `scripts/build-html-book.mjs` or reproduce its measured text-page pagination pattern in your custom build. Do not hand-write each chapter as `<section class="chapter-flow flow-single">` and then add designed feature pages around it; that bypasses the `.text-frame` overflow checks and creates article-like or too-narrow pages instead of a book interior. If a custom build intentionally uses long `.chapter-flow` prose for a plain reader or appendix, add `data-allow-flowing-prose="true"` to the `.book` root or that specific flow section and verify the rendered measure is deliberate.
...
- Treat long `.chapter-flow` body prose as opt-in only. Without `data-allow-flowing-prose="true"`, the verifier will reject substantial unmeasured `.chapter-flow` content, especially `.flow-single` sections whose padding leaves an accidentally skinny text measure.
...
13. **Body measure check** - For editorial/coffee-table chapters, inspect representative body pages. Dense body text should be measured `.text-page` prose, usually two-column or a deliberate single-column text page; it must not be a full-width slab, an unmeasured `.chapter-flow`, or a skinny accidental strip. If the verifier reports missing measured text pages, long unmeasured chapter flow, or narrow chapter-flow measures, the book is incomplete unless it is an intentional plain-reader/appendix section marked with `data-allow-flowing-prose="true"`.
