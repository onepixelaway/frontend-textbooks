#!/usr/bin/env bash
# deploy.sh — Deploy a Frontend Textbooks HTML book to Vercel for sharing
#
# Usage:
#   bash scripts/deploy.sh <path-to-book-folder-or-html>
#
# Examples:
#   bash scripts/deploy.sh ./my-book/
#   bash scripts/deploy.sh ./book.html
#
# What this does:
#   1. Checks if Vercel CLI is installed (installs if not)
#   2. Checks if user is logged in (guides through login if not)
#   3. Deploys the HTML book to a public URL
...
if [[ $# -lt 1 ]]; then
    err "Usage: bash scripts/deploy.sh <path-to-book-folder-or-html>"
    err ""
    err "Examples:"
    err "  bash scripts/deploy.sh ./my-book/"
    err "  bash scripts/deploy.sh ./book.html"
    exit 1
fi
...
if [[ -f "$INPUT" && "$INPUT" == *.html ]]; then
    ...
    if [[ ! -f "$INPUT/index.html" ]]; then
        ...
        err "Make sure your book folder has an index.html."
        exit 1
    fi
    DEPLOY_DIR="$INPUT"
    CLEANUP_TEMP=false
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════╗${NC}"
echo -e "${BOLD}║       Deploy Book to Vercel           ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════╝${NC}"
echo ""
...
BOOK_NAME=$(basename "$DEPLOY_DIR")
# If we used a temp dir, use the original filename without .html
if [[ "$CLEANUP_TEMP" == "true" ]]; then
    BOOK_NAME=$(basename "$INPUT" .html)
fi

# Sanitize project name for Vercel:
# - lowercase, replace spaces/special chars with hyphens
# - collapse multiple hyphens, trim to 100 chars
BOOK_NAME=$(echo "$BOOK_NAME" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9._-]/-/g' | sed 's/--*/-/g' | sed 's/^-//;s/-$//' | cut -c1-100)

# Vercel uses the directory name as the project name, so rename the deploy
# directory to the sanitized book name (avoids deprecated --name flag)
if [[ "$CLEANUP_TEMP" == "true" ]]; then
    RENAMED_DIR="$(dirname "$DEPLOY_DIR")/$BOOK_NAME"
...
echo -e "       and delete the project '${BOOK_NAME}'."
echo -e "${BOLD}════════════════════════════════════════${NC}"
echo ""
