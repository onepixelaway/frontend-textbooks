...
export function themeImagePrompt(theme, subject, constraints) {
  const contract = theme?.imagePrompt;
  if (!contract?.template || !contract?.subjectPlaceholder || !contract?.constraintPlaceholder) {
...
export function createCoverImageRequest({ config, plan, outputDir }) {
  ...
  const value = {
    ...
    prompt: themeImagePrompt(theme, cover.subject, constraints),
    constraints
  };
