...
Five selected Dribbble pairing themes are registered in `themes/dribbble-pairings.mjs`. Each records its article attribution, original pairing, bundled pairing, immutable font sources, full semantic palette, and any license-driven substitution. See `themes/FONT_SOURCES.md` for the provenance ledger. Their canonical image prompts are intentionally `null` until supplied by the user. Selecting one as a `fontTheme` is immediately supported; selecting one as the visual `style` makes cover request generation fail closed until its prompt contract is filled.
