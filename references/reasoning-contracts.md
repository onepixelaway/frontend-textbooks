...
Use version 2 of `schemas/book-plan.schema.json`. Include editorial intent, registered theme and rationale, body layout and rationale, visual policy, the mandatory `visuals.cover` decision, salient classifications, explicit exceptions, and subjective aesthetic criteria. Reference only IDs from `planning-inventory.json`. Do not copy manuscript paragraphs into rationales or diagram labels.
